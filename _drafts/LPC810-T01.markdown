...
code_root: repo/00-Getting_Started
---
...
Unpack the file somewhere you can remember. Plug the TTL-USB cable in(with the breadboard, of course). Hold the reset button, then hold the ISP button. Still holding ISP button, execute
...
That's it for the `main.c`. Simple. However, with only this source file, the linker can't know where to put the code in the executable. Therefore, we need to tell it where to put what. Now let's look at/*  the `LPC810/lpc810.ld`.

{% highlight LD linenos %}
{% include {{ code_root }}/LPC810/lpc810.ld %}
{% endhighlight %}

An executable is divided into multiple sections. Each section is placed into a storage device in the microcontroller. The starting address and size are documented in datasheet, in section *Memory Map*. In our case, an LPC810 has 4K flash and 1K RAM, starting at address `0x00000000` and `0x10000000`, therefore, we have the following snippet of LD script(ignore the `ENTRY()` for now):
...
Since Cortex-M0+ is 32 bits, each instruction is 4 bytes wide. Therefore, we need to align the code to instructions before advancing to next section. If it is not aligned, the linker just appends `nop` (that is an observation, not completely sure).

	. = 0xC4;

This line is just for this tutorial because otherwise you have to type a whole list of 48 vectors and create 48 function definitions before you can even start blinking an LED. We all know from above that the actual executable starts at `0xC4`, so this is totally fine(and I have tested it, so really, it is fine).
